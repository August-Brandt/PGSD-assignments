...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArg) -> 
      ...
      | Closure (f, x, fBody, fDeclEnv) ->
        let zipArgs = List.zip x eArg
        let fBodyEnv = (f, fClosure) :: fDeclEnv
        let fBodyEnv = List.fold (fun acc elem -> (fst elem, Int(eval (snd elem) env)) :: acc) fBodyEnv zipArgs
        eval fBody fBodyEnv
...
let ex2 = Letfun("f1", ["x";"y"], Prim("+", Var "x", Var "y"), 
                 Call(Var "f1", [CstI 12;CstI 4]));;
