...
int execcode(word p[], word s[], word iargs[], int iargc, int /* boolean */ trace) {
  ...
  for (;;) {
    ...
    case SETCAR: {
      word v = (word)s[sp--];
      word* p = (word*)s[sp];
      p[1] = v; // maybe oh oh poopy stinky should decrement sp according to schema
    } break;
    case SETCDR: {
      word v = (word)s[sp--];
      word* p = (word*)s[sp];
...
void mark(word *block) {
  printf("Painting the town black\n");
  if (Color(block[0]) == Black) {
    printf("It's already black\n");
    return;
  }
  ...
  for (int i = 1; i < len+1; i++) {
    word v = block[i];
    if (!IsInt(v) && inHeap((word*)v) && v != 0 ) {
      mark((word*)v);
    }
  }

  // printf("Length: %d\n", Length(block[0]));
  // for (int i = 1; i < Length(block[0])+1, i++;) {
  //   // printf("Is int: %d\n", IsInt(block[i]));
  //   word v = block[i];
  //   if (!IsInt(v) && inHeap((word*)v) && v != 0 ) {
  //     printf("%d Stuff is becoming black\n", i);
  //     mark((word*)v);
  //   }
  // }
}
...
void sweepPhase() {
  printf("sweeping ....\n");
  word* current = freelist;
  if (current != 0) {
    while (current[1] != 0) {
      current = (word*)current[1];
    }
  }
  ...
  while(inHeap(i)) {
    ...
    if (color == White) {
      while (1) {
        word nextBlock = i[length+1];
        if (Color(nextBlock) == Blue || Color(nextBlock) == White) {
          length += Length(nextBlock)+1;
        } else {
          break;
        }
      }
      *i = mkheader(0, length, Blue);
      if (current == 0) {
        current = i;
        current[1] = 0;
        freelist = current;
